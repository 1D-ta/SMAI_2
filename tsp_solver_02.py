...
def nearest_tsp(cities, start=None, use_matrix=False) -> Tour:
    """Generate a tour using the nearest neighbor heuristic for Non-Euclidean or Euclidean cases."""
    ...
    if use_matrix:
        ...
        # Euclidean case (original implementation)
        while unvisited:
...
def reversal_is_improvement(tour, i, j, use_matrix=False, city_indices=None) -> bool:
    ...
    Use matrix or Euclidean distance as needed."""
    A, B, C, D = tour[i-1], tour[i], tour[j-1], tour[j % len(tour)]
    if use_matrix:
        # Use distance matrix lookups
...
def read_input():
    """Reads the input for TSP, including cities and optionally a distance matrix."""
...
def main():
    """Main function to solve TSP for both Euclidean and Non-Euclidean cases."""
    tsp_type, cities, distances = read_input()  # Read input, including distance matrix.
    ...
    # Calculate the tour length, considering matrix or not
    ...
    # Optional: Brute force solution (commented out for large cases)
    '''
